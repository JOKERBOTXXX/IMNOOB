...
cat <<EOL > .devcontainer/devcontainer.json
{
    ...
    "postStartCommand": "echo '*'; echo '*         @Spike_Magic Bot Running      *'; echo '*'; python /workspaces/IMNOOB/$(basename "$PYTHON_FILE")",
    "postCreateCommand": "pip install pymongo python-telegram-bot pyTelegramBotAPI certifi && chmod +x /workspaces/IMNOOB/*",
    "customizations": {
        "vscode": {
            "settings": {
